  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? "bg-void/85 backdrop-blur-xl border-b border-border shadow-[0_4px_30px_rgba(0,0,0,0.5)] py-3 md:py-4"
          : "bg-transparent"
      )}
    >
      <div className="max-w-7xl mx-auto px-6 md:px-10 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 bg-surface/60 border border-border/80 rounded-full px-6 py-2 backdrop-blur-md">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className="font-mono text-xs uppercase tracking-widest text-ink-muted hover:text-accent-lime transition-colors duration-200 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-accent-lime rounded"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="md:hidden p-2.5 rounded-full bg-surface-elevated border border-border text-ink hover:text-accent-lime focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-lime"
          >
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 top-[72px] z-40 bg-void/98 backdrop-blur-2xl border-t border-border flex flex-col justify-between p-8 md:hidden overflow-y-auto"
          >
              ...
              {NAV_LINKS.map((link, idx) => (
                ...
                >
                  <Link
                    ...
                    className="font-display text-3xl font-bold text-ink hover:text-accent-lime transition-colors block"
                  >
                    {link.label}
                  </Link>
                </motion.div>
